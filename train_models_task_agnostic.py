...
CONFIG.setdefault('train_on_all_tasks', True)

# output folders (refactored suffixes)
OUT_BASE = Path(CONFIG.get('output_folder', './results_task_agnostic'))
PLOTS_DIR = OUT_BASE / (Path(CONFIG.get('plots_folder', 'plots')).name + '_task_agnostic')
IMP_DIR = OUT_BASE / (Path(CONFIG.get('importance_folder', 'importances')).name + '_task_agnostic')
SHAP_DIR = OUT_BASE / (Path(CONFIG.get('shap_folder', 'shap')).name + '_task_agnostic')
...
def evaluate_task_window(file_path: Path, window_label: str):
    ...
    for task in tasks:
        logger.info(f'Evaluating Task (trained on ALL tasks): {task} (window: {window_label})')
