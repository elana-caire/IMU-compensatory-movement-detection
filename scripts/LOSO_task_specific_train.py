...
def inner_tune_params(df_task: pd.DataFrame, features: List[str], groups: np.ndarray,
    ...
    max_evals = MAX_INNER_EVALS
    grid = combos
    if USE_RANDOM_SEARCH and len(grid) > max_evals:
    ...
    for task in tasks:
        ...
        for model_name in MODELS:
            ...
            for test_subject in unique_subjects:
                ...
                param_grid_input = PARAMS_GRID.get(model_name, {})
...
def main():
    ...
    if all_results:
        ...
        results_df.to_csv(PLOTS_DIR / 'model_results_LOSO_task_specific.csv', index=False)
    else:
