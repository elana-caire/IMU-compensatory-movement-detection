...
if loso_config.BEST_MODEL == "task_agnostic":
    models_summary_path = paths_config.MODELS_PATH / 'model_results_LOSO_task_agnostic.csv'
    task_specific = False
    print("LOSO Task-agnostic mode selected for explainability.")
elif loso_config.BEST_MODEL == "task_specific":
    models_summary_path = paths_config.MODELS_PATH / 'model_results_LOSO_task_specific.csv'
    task_specific = True
    print("LOSO Task-specific mode selected for explainability.")
else:
    raise ValueError(f"Invalid BEST_MODEL value: {loso_config.BEST_MODEL}")
...
COMPUTE_PERMUTATION_IMPORTANCE = loso_config.COMPUTE_PERMUTATION_IMPORTANCE
COMPUTE_SHAP = loso_config.COMPUTE_SHAP
BG_N = loso_config.SHAP_SAMPLE_SIZE
PERM_N_REPEATS = loso_config.PERM_N_REPEATS
PERM_SCORING = loso_config.PERM_SCORING
BEST_MODELS_FOR_EXPLAINABILITY = loso_config.BEST_MODELS_FOR_EXPLAINABILITY
...
if __name__=='__main__':
    ...
    for file_path, window_size in tqdm(feature_files, desc="Processing windows"):
        if window_size == "750":
            ...
            df_best = df_best[df_best["model_name"].isin(BEST_MODELS_FOR_EXPLAINABILITY)]
            ...
            for task_name in df_best["task"].unique():
                ...
                for curr_model in BEST_MODELS_FOR_EXPLAINABILITY:
                    if curr_model!='MLP':
                    ...
                    for subject in subjects:
                        ...
                        model_out_dir = os.path.join(BEST_MODELS_PATH, "saved_models", task_name, curr_model)
                    ...
                    df_shap_subj.to_csv(os.path.join(BEST_MODELS_PATH, f"shap_{task_name}_{curr_model}_test.csv"), index=False)
                    ...
                    df_shap_agg.to_csv(os.path.join(BEST_MODELS_PATH, f"shap_{task_name}_{curr_model}_mean_std.csv"), index=False)

                    df_perm_subj = pd.DataFrame(per_subject_perm)
                    df_perm_subj.to_csv(os.path.join(BEST_MODELS_PATH, f"perm_{task_name}_{curr_model}_test.csv"), index=False)
                    ...
                    df_perm_agg.to_csv(os.path.join(BEST_MODELS_PATH, f"perm_{task_name}_{curr_model}_mean_std.csv"), index=False)
                    ...
                    df_metrics_agg.to_csv(os.path.join(BEST_MODELS_PATH, f"metrics_{task_name}_{curr_model}_mean_std.csv"), index=False)